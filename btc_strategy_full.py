...
def log_to_csv(date, price, signal, action, entry_price, exit_price, profit, reasons, indicators):
    ...
    with open(CSV_FILE, 'a', newline='') as f:
        fieldnames = ['Date', 'BTC Price', 'Signal', 'Action', 'Entry Price', 
...
def generate_price_chart(df, symbol="BTC/USDT"):
    ...
    try:
        ...
        ax.set_title(f'{symbol} - 2 Hour Chart', fontsize=16, fontweight='bold')
...
def get_crypto_price_chart(symbol, duration="24h"):
    """Use crypto-price skill to get chart data"""
    try:
        cmd = ['python3', '/home/ironman/.openclaw/workspace/skills/crypto-price/scripts/get_price_chart.py', symbol, duration]
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
        
        if result.returncode == 0:
            # Parse JSON output
            data = json.loads(result.stdout)
            return data.get('chart_path'), data.get('text_plain')
        else:
            print(f"Chart command failed: {result.stderr}")
            return None, None
    except Exception as e:
        print(f"Chart command error: {e}")
        return None, None

def send_report_with_chart(report_text, chart_path):
    """Send report with chart via message tool"""
    try:
        # Write report to a temporary file
        with open('/tmp/btc_report.txt', 'w') as f:
            f.write(report_text)
        
        # Send the report with chart
        subprocess.run([
            'python3', '-c', 
            f'''
import subprocess
...
# Send text report
subprocess.run([
    "python3", "/home/ironman/.openclaw/workspace/skills/message/message.py",
    "send",
    "--channel", "telegram",
    "--to", "195050411",
    "--message", """{report_text}"""
